...
import com.huang.base.utils.IdWorkerUtils;
import com.huang.base.utils.QRCodeUtil;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        String url = String.format(rawUrl, payNo);
        String qrCode = null;
        try {
...
//校验失败
                log.info("校验支付结果失败,支付记录:{},APP_ID: {},totalPrice: {} "
                        , payRecord.toString(), payStatusDto.getApp_id(), total_amount.intValue());
                XueChengPlusException.cast("校验支付结果失败");
            }
            log.debug("更新支付结果,支付交易流水号:{},支付结果:{}",
                    payNo, trade_status);
